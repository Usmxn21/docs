...
app.post('/webhooks/revtain', express.raw({ type: 'application/json' }), (req, res) => {
  const signature = req.headers['x-revtain-signature'];
  const webhookSecret = process.env.REVTAIN_WEBHOOK_SECRET;

  // req.body is a Buffer (the raw bytes Revtain sent). Hash it directly.
...
def handle_webhook():
    ...
    webhook_secret = os.environ['REVTAIN_WEBHOOK_SECRET']

    # Hash the RAW request body — the exact bytes Revtain sent.
    ...
    print(f"Received verified webhook: {payload.get('event')}")

    return 'OK', 200
```

```bash cURL (Test)
# Reproduce a signature locally. The body passed to openssl must be byte-identical
# to what your server receives, so sign and send the SAME string.
...
**Action:** Branch on `recommendedAction` (see table below) and run your own customer outreach from your domain — see the [dunning guide](/guides/dunning). Revtain never emails or messages your customers directly.
...
**Action:** Consider reaching out to the customer to update their payment method before the next billing cycle. Generate a card update link via [`/api/recovery/update-card/generate`](/api-reference).
...
Sent when a customer completes the hosted [cancel flow](/api-reference). The event name carries the outcome: `churn.flow.retained`, `churn.flow.paused`, `churn.flow.downgraded`, or `churn.flow.cancelled`.
